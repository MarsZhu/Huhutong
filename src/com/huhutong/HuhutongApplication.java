    ...
    static class MyGeneralListener implements MKGeneralListener {
        ...
        public void onGetNetworkState(int iError) {
            if (iError == MKEvent.ERROR_NETWORK_CONNECT) {
                Toast.makeText(HuhutongApplication.getInstance().getApplicationContext(), "您的网络出错啦！",
                    Toast.LENGTH_LONG).show();
            }
            else if (iError == MKEvent.ERROR_NETWORK_DATA) {
                Toast.makeText(HuhutongApplication.getInstance().getApplicationContext(), "输入正确的检索条件！",
                        Toast.LENGTH_LONG).show();
            }
            // ...
        }
        ...
        public void onGetPermissionState(int iError) {
            ...
            if (iError != 0) {
                //授权Key错误：
                Toast.makeText(HuhutongApplication.getInstance().getApplicationContext(), 
                        "请在 HuhutongApplication.java文件输入正确的授权Key,并检查您的网络连接是否正常！error: "+iError, Toast.LENGTH_LONG).show();
